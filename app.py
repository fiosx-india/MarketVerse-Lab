...
st.set_page_config(page_title="MarketVerse Lab", page_icon="🚀", layout="wide")
...
st.json(
    {"Guardian": "🟢 Ready", "Scanner": "🟢 Ready", "Reports": "🟢 Ready", "AI": "🟢 Ready"}
)
...
with col2:
    ...
    report_limit = st.selectbox(
        "Report Line Limit", [100, 500, 1000, 1500, 5000], index=2
    )

    download_limit = st.selectbox(
        "Download Line Limit", [100, 500, 1000, 1500, 5000], index=2
    )

    copy_limit = st.selectbox("Copy Line Limit", [100, 500, 1000, 1500], index=2)

    keep_reports = st.selectbox("Keep Last Reports", [1, 5, 10, 20], index=3)

    clear_time = st.selectbox(
        "Auto Delete Reports", ["Never", "1 Hour", "6 Hours", "24 Hours"], index=2
    )
...
if st.button("🔍 Scan Project", use_container_width=True):
    ...
    try:
        ...
        col4.metric("Modules", f'{score["ready_modules"]}/{score["total_modules"]}')
...
with col1:
    st.metric("🛡️ Guardian", report.get("name", "Guardian Core"))

with col2:
    st.metric("📦 Version", report.get("version", "1.0.0"))
...
with col2:
    st.metric("🧩 Ready Modules", f"{report['ready_modules']}/{report['total_modules']}")
...
for module, state in report["modules"].items():
    ...
    module_table.append(
        {"Module": module, "Status": "✅ Ready" if state else "❌ Waiting"}
    )
...
for module, state in report["modules"].items():
    ...
    report_text += f"{module:<30}" f"{'✅ Ready' if state else '❌ Waiting'}\n"
...
st.text_area("Guardian Core Report", report_text, height=350)
...
try:
    ...
    st.text_area("Guardian Diagnostic Report", report_text, height=500)
    ...
    if "score" in report:
        ...
        st.metric("Guardian Score", score["score"])

        st.success(f"Signal : {score['signal']}")
