...
try:
    ...
    with tab3:
        if guardian:
            ...
            action = st.selectbox("Action", ["modify", "create", "delete"])
            if st.button("Simulate"):
                st.json(guardian.simulate_change(file_name, action))
            ...
            st.subheader("State Manager")
            state = st.selectbox("Project State", ["IDLE", "RUNNING", "RECOVERING", "FAILED", "COMPLETED"])
            if st.button("Update State"):
                st.json(guardian.set_state(state))

            st.subheader("Audit Manager")
            audit_action = st.text_input("Audit Action")
            audit_details = st.text_input("Audit Details")
            ...
            level = st.selectbox("Level", ["INFO", "SUCCESS", "WARNING", "ERROR"])
            if st.button("Send Notification"):
                st.json(guardian.notify(title, message, level))

            st.subheader("Notification History")
            st.json(guardian.notification_history())
            ...
            st.json(guardian.pending_tasks())
            ...
            st.json(guardian.policy_history())

            remove_name = st.text_input("Remove Policy", key="remove_policy")
            if st.button("Delete Policy"):
                st.json(guardian.remove_policy(remove_name))
            ...
            st.json(guardian.event_history())
            ...
            st.json(guardian.resource_status())
